...
public static class MinimaxAIController 
{
    ...
    {
        if (CheckGameWin(GameManager.PlayerType.PlayerA, board)) return -10 + depth;
        if (CheckGameWin(GameManager.PlayerType.PlayerB, board)) return 10 - depth;
        if (IsAllBlocksPlaced(board)) return 0;
    ...
    /// </summary>
    /// <returns></returns>
    public static bool IsAllBlocksPlaced(GameManager.PlayerType[,] _board)
    {
        for (var row = 0; row < _board.GetLength(0); row++)
        {
            for (var col = 0; col < _board.GetLength(1); col++)
            {
                if (_board[row, col] == GameManager.PlayerType.None)
                {
                    return false;
                }
            }
