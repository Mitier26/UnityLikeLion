...
public class UIManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        fadeAction += () =>
        {
            GamePanels[(int)GameManagerSingle.Instance.e_GameType].SetActive(false);
            // GameManagerSingle.Instance.OnChangeType(GameManagerSingle.Instance.e_GameType + 1);
            GameManagerSingle.Instance.OnChangeType(GameManagerSingle.GameType.BUILD);
            GamePanels[(int)GameManagerSingle.Instance.e_GameType].SetActive(true);
        };
        
        fadeButtons[(int)GameManagerSingle.Instance.e_GameType].onClick.AddListener(FadeEvent);

        foreach (Button fadeButton in fadeButtons)
        {
            fadeButton.onClick.AddListener(FadeEvent);
        }
    }
